...
function craftBuyLink() {
	const lang_src = currentLangSrc();
	const lang_dst = currentLangDst();
	const buy_url = buy_link.dataset.url;
	const customer_id = buy_link.dataset.cid;

	buy_link.href = `${buy_url}?client_reference_id=${customer_id}-${lang_src}-${lang_dst}`;
}
